...
# 匹配方式的"或"：
#   (a) 同一行内任意字符（`.{0,256}`）——保留同一行里"关键词之间夹杂任意
#       字符"都应该命中的原有行为，例如 `vssadmin.exe delete shadows`、
#       带完整路径的 `C:\Windows\System32\vssadmin.exe delete shadows`。
#       256 这个长度上限足够覆盖真实场景里再长的单行命令（关键词之间顶多
#       夹一段路径/几个参数），同时是下面这条 ReDoS 修复的关键：不加上限的
#       `.*` 在这个位置会有灾难性回溯——构造
#       `("vssadmin delete " * n) + "X"` 这种"关键词重复出现但永远凑不出
#       完整匹配"的输入，回溯耗时随 n 呈多项式级增长（跟输入是否恶意构造
#       无关，LLM 生成代码进入"重复退化"这种真实失败模式就可能意外撞上）。
#       `pre_tool_call` 钩子同步跑在每次工具调用之前，卡死几秒到几十分钟
#       对用户来说等同于"软件坏了"。加上 256 长度上限后，同样的对抗输入
#       从秒级/分钟级降到两位数毫秒级（数量级差异来自：一旦长度有界，单个
#       起始位置的最坏回溯次数就是跟输入长度无关的常数，总耗时随输入长度
#       线性增长，而不是随长度的更高次幂增长）。
#       代价（已知、可接受）：同一行里如果关键词之间塞了超过约 256 个任意
#       字符的填充物，会落回"跨行胶水"分支（只认胶水字符）而匹配失败，
#       不再像旧版无界 `.*` 那样必定命中——但真实的 vssadmin/wbadmin/cipher
#       命令语法里，这几个关键词之间从来不会隔这么远，会刻意塞这么多填充
#       字符的场景本身也大概率同时破坏了这条命令在 shell/cmd.exe 里的真实
#       可执行性，不是一个需要专门堵的现实威胁；
#       见 test_known_limitation_long_same_line_padding_bypasses_connector_bound。
...
_CONNECTOR = r"(?:.{0,256}|" + _LINE_SPLIT_GLUE + r")"
...
_RULES: list[tuple[str, str]] = [
    ...
    # 到此为止。只要盘符冒号后（跳过可选的空白）紧跟的不是这几种之一，就
    # 判定为普通文字而不匹配——不管后面紧跟的是普通单词还是任意标点。
    #
    # "跳过可选的空白"用 `[^\S\r\n]*`（"空白但不是 \r/\n"）而不是
    # `[ \t]*`：这个产品默认中文界面，全角空格（U+3000）、不换行空格
    # （NBSP/U+00A0）在中文输入法/中文软件生成的文本里很常见，是正常使用
    # 中就有真实概率出现的字符，不是刻意构造的对抗输入——`format C:　/q`
    # （全角空格）、`format C: /q`（NBSP）这类命令曾经因为只认 ASCII 的
    # `[ \t]` 而漏检。Python 的 `\s` 在没有 `re.ASCII` 标志时本身就是
    # Unicode-aware（含 U+3000/U+00A0 等），`[^\S\r\n]` 等价于"用 `\s` 的
    # 全部 Unicode 覆盖面，但排除 `\r`/`\n`"——换行必须继续留给下面专门的
    # `\r?\n` 分支处理，不能被这里的空白类顺带吃掉，否则会改变换行分支的
    # 匹配语义。
    ("format 磁盘格式化", r"\bformat\b\s+[a-zA-Z]:(?:[\\/]|[^\S\r\n]*(?:[/&;|<>]|\r?\n|$))"),
